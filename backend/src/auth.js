...
async function requireAuth(pool, req) {
    return authenticate(pool, req);
}

function createAuthorizationGuard({ roles = [], permissions = [] } = {}) {
    return async req => {
        const auth = req.auth;
        if (!auth) {
...
function checkPermission(...requiredPermissions) {
    return req => {
        ...
        const authorized = requiredPermissions.every(permission => userPermissions.includes(permission));
...
function ownerCheck({ anyPermission, ownerResolver }) {
    return async req => {
        ...
        if (!ownerId || ownerId !== req.auth?.id) {
            throw createHttpError(403, 'Vous ne pouvez accéder qu’à vos propres données.');
        }
    };
}
