...
export function Navbar({ onAuthenticate }: { onAuthenticate?: () => void }) {
  ...
  async function logout() { try { await authService.logout() } catch { /* Le logout local reste possible. */ } clearSession(); close(); navigate('/', { replace: true }) }
  ...
  return <header className="sticky top-0 z-50 border-b border-slate-100 bg-white/95 backdrop-blur">
    <div className="mx-auto flex h-20 max-w-7xl items-center justify-between gap-4 px-6">
      ...
      <div className="relative flex items-center gap-3">
        {!authenticated ? <button type="button" onClick={onAuthenticate} className="rounded-xl border border-slate-300 px-4 py-2 text-sm font-bold text-slate-700 transition hover:border-emerald-600 hover:text-emerald-800">Se connecter / S’inscrire</button> : <>
