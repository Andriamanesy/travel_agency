...
export function ResourceManager({ resource }: { resource: BackofficeResource }) {
  ...
  return (
    <section className="min-h-screen bg-slate-50/60 dark:bg-[#09090b] pb-20 transition-colors">
      {/* En-tête de section */}
      <header className="border-b border-slate-200/80 dark:border-slate-800 bg-white dark:bg-[#121214] px-8 py-6 shadow-sm transition-colors">
        <div className="mx-auto max-w-7xl flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <h1 className="mt-1 text-2xl font-black text-slate-900 dark:text-white md:text-3xl">{config.title}</h1>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{config.intro}</p>
          </div>
      ...
      <div className="mx-auto max-w-7xl px-6 pt-8 grid gap-8 xl:grid-cols-[420px_1fr]">
        ...
        {/* Formulaire latéral */}
        <form onSubmit={submit} className="h-fit rounded-3xl bg-white dark:bg-[#121214] p-6 shadow-sm border border-slate-200/80 dark:border-slate-800 sticky top-6 transition-colors">
          <div className="mb-6 flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
            <div className="flex items-center gap-2.5">
              <div className={`h-3 w-3 rounded-full ${editing ? 'bg-amber-500 ring-4 ring-amber-100 dark:ring-amber-950/50' : 'bg-emerald-500 ring-4 ring-emerald-100 dark:ring-emerald-950/50'}`} />
              <h2 className="text-base font-black text-slate-900 dark:text-white">
                {editing ? 'Modifier l’élément' : 'Nouveau contenu'}
              </h2>
            </div>
            {editing && (
              <span className="rounded-full bg-amber-100 dark:bg-amber-950/60 px-3 py-1 text-[10px] font-black uppercase tracking-wider text-amber-800 dark:text-amber-300">
          ...
          <div className="mt-8 flex gap-3 border-t border-slate-100 dark:border-slate-800 pt-5">
            {editing && (
              <button 
                type="button" 
                onClick={() => { setEditing(null); form.reset(defaults(resource)); }}
            ...
            <button 
              ...
              className="flex-1 inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-700 dark:bg-emerald-600 px-4 py-3 text-xs font-bold text-white shadow-md shadow-emerald-700/20 transition hover:bg-emerald-800 dark:hover:bg-emerald-500 disabled:opacity-50 cursor-pointer"
            >
              <Plus size={16} />
        ...
        {/* Liste des éléments */}
        <div className="rounded-3xl bg-white dark:bg-[#121214] shadow-sm border border-slate-200/80 dark:border-slate-800 overflow-hidden flex flex-col transition-colors">
          
          {/* Barre d'outils et de filtres */}
          <div className="flex flex-col gap-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/40 p-6 sm:flex-row sm:items-center sm:justify-between transition-colors">
            ...
            <div className="flex items-center gap-3">
              <div className="relative flex-1 sm:w-64">
                ...
                <input 
                  ...
                  className="w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 py-2 pl-9 pr-8 text-xs font-medium text-slate-900 dark:text-white outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10"
                />
                {searchQuery && (
          ...
          {selectedIds.length > 0 && (
            <div className="flex items-center justify-between bg-emerald-50 dark:bg-emerald-950/40 px-6 py-3 border-b border-emerald-100 dark:border-emerald-900/50">
              <span className="text-xs font-bold text-emerald-900 dark:text-emerald-300">{selectedIds.length} élément(s) sélectionné(s)</span>
              <button 
                onClick={() => {
          ...
          <div className="flex-1 p-6">
            {isPending ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 text-slate-400 text-center">
                <AlertCircle size={44} className="mb-3 text-slate-300 dark:text-slate-700" />
                <p className="font-bold text-slate-700 dark:text-slate-200 text-base">Aucun résultat trouvé</p>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {filteredRows.map((row) => (
                  <article key={String(row.id)} className="group relative flex flex-col justify-between rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 p-5 shadow-sm transition hover:border-emerald-300 dark:hover:border-emerald-700 hover:shadow-md">
                    <div>
                      ...
                      <h4 className="mt-3 font-bold text-slate-900 dark:text-white group-hover:text-emerald-700 dark:group-hover:text-emerald-400 transition line-clamp-1">
                    ...
                    <div className="mt-4 border-t border-slate-100 dark:border-slate-800 pt-3 flex items-center justify-between text-[11px] font-medium text-slate-400">
                      <span>ID #{String(row.id).slice(0, 8)}</span>
                      {Boolean(row.cover_image || row.image_url) && (
                        <span className="inline-flex items-center gap-1 font-semibold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/60 px-2 py-0.5 rounded-md">
...
function ResourceField({ field, form }: { field: Field; form: UseFormReturn<Record<string, unknown>> }) { 
  ...
  if (field.type === 'checkbox') {
    return (
      <label className="flex cursor-pointer items-center justify-between rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-900/40 p-3.5 transition hover:bg-slate-100 dark:hover:bg-slate-800/60">
        <span className="text-xs font-bold text-slate-700 dark:text-slate-300">{field.label}</span>
        <input type="checkbox" className="h-4 w-4 rounded border-slate-300 dark:border-slate-700 text-emerald-600 focus:ring-emerald-600 cursor-pointer bg-transparent" {...form.register(field.key)} />
      </label>
    )
  }
  ...
  if (field.type === 'image') {
    ...
    return (
      <div className="w-full space-y-2">
        <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">{field.label}</label>
        
        {imageUrl ? (
          <div className="relative overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 h-32 bg-slate-100 dark:bg-slate-900 group">
          ...
          <div 
            ...
            className="flex flex-col items-center justify-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl p-6 bg-slate-50/50 dark:bg-slate-900/40 hover:bg-slate-100/60 dark:hover:bg-slate-900/85 transition cursor-pointer text-center"
          >
            <Upload size={24} className="text-slate-400 mb-2" />
            <p className="text-xs font-bold text-slate-700 dark:text-slate-300">Cliquez pour téléverser une image</p>
        ...
        <input 
          ...
          className="w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-3.5 py-2 text-xs text-slate-900 dark:text-white outline-none transition focus:border-emerald-500" 
          {...form.register(field.key)} 
        />
        {error && <span className="text-[11px] font-medium text-red-600 dark:text-red-400">{error}</span>}
      </div>
    )
  }
  ...
  return (
    <div className="w-full space-y-1">
      ...
      {field.type === 'textarea' ? (
        <textarea className="w-full min-h-[85px] rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-3.5 py-2.5 text-xs text-slate-900 dark:text-white outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10" {...form.register(field.key)} />
      ) : field.type === 'richtext' ? (
        ...
        <input className="w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-3.5 py-2.5 text-xs text-slate-900 dark:text-white outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10" type={field.type || 'text'} {...form.register(field.key)} />
      )}
