...
export function AuthModal({ open, initialMode = 'login', redirectTo = '/dashboard', onClose, onToast, onAuthenticated, onNotice }: AuthModalProps) {
  ...
  const onRegister = (values: RegisterValues) => registerAccount.mutate({ name: `${values.firstName} ${values.lastName}`, email: values.email, password: values.password }, { onSuccess: () => { notify('Compte créé. Vérifiez votre e-mail pour l’activer.'); onClose() } })
