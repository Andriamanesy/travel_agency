...
export function CatalogPage() {
  const { entity = '' } = useParams(); const [input, setInput] = useState(''); const [search, setSearch] = useState('')
  const isValidEntity = catalogEntities.includes(entity as CatalogEntity)
  const catalogEntity: CatalogEntity = isValidEntity ? entity as CatalogEntity : 'circuits'; const { data, isPending, isError } = useCatalog(catalogEntity, search)
  if (!isValidEntity) return <Navigate to="/catalog/circuits" replace />
  const submit = (event: FormEvent) => { event.preventDefault(); setSearch(input) }
  return <main className="mx-auto max-w-7xl px-6 py-12"><Link to="/" className="font-semibold text-emerald-700">← TravelMS</Link><p className="mt-8 text-xs font-bold tracking-widest text-emerald-700">{labels[catalogEntity][0]}</p><h1 className="mt-2 text-4xl font-black">{labels[catalogEntity][1]}</h1><form onSubmit={submit} className="mt-8 flex max-w-xl gap-3"><input value={input} onChange={(event) => setInput(event.target.value)} className="min-w-0 flex-1 rounded-xl border border-slate-200 bg-white px-4 py-3" placeholder="Rechercher" /><button className="rounded-xl bg-slate-900 px-5 font-semibold text-white">Rechercher</button></form>{isPending && <p className="mt-8">Chargement…</p>}{isError && <p role="alert" className="mt-6 text-red-700">Impossible de charger le catalogue.</p>}<div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{data?.[catalogEntity]?.map((item) => <CatalogCard key={item.id} item={item} entity={catalogEntity} />)}</div>{data?.[catalogEntity]?.length === 0 && <p className="mt-10">Aucun résultat pour le moment.</p>}</main>
}
