...
const API_BASE_URL = window.TravelConfig.apiBaseUrl;
    ...
    avatarFileInput.addEventListener('change', (e) => {
        ...
        if (file) {
            ...
            img.src = previewUrl;
            // Libération de la mémoire une fois chargée
            img.onload = () => URL.revokeObjectURL(previewUrl);
        }
    });

    // 3. VÉRIFICATION DYNAMIQUE DU STATUT DE PROFIL
    ...
    function checkProfileCompletion() {
        ...
        if (isComplete) {
            badge.className = "bg-emerald-50 text-emerald-700 px-4 py-2 rounded-2xl text-xs font-bold uppercase tracking-wider transition-all duration-300";
            badge.textContent = "Profil Complet";
        } else {
            badge.className = "bg-amber-50 text-amber-700 px-4 py-2 rounded-2xl text-xs font-bold uppercase tracking-wider transition-all duration-300";
            badge.textContent = "Profil Partiel";
        }
    }

    // Bloquer tout caractère non numérique dans le téléphone
    ...
    async function loadCountries() {
        ...
        // Le formulaire reste immédiatement utilisable, même sans Internet.
        ...
        try {
            // Même origine : le backend gère RestCountries, son cache et le délai d'attente.
            const response = await fetch('/api/countries');
            if (!response.ok) throw new Error('Erreur réseau');
            const { countries } = await response.json();
            renderCountries(countries);
        } catch (error) {
            // La liste locale est déjà affichée : aucune erreur à montrer à l'utilisateur.
        }
    }
